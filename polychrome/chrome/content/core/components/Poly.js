...
*/
PolyChrome.Poly = function(document, console) {
    this.document = document;
    this.console = console;
    this.status = {s:""};
}
PolyChrome.Poly.prototype = {
    process : null,
    document : null,
    enabled : false,
    ready : false,
    ...
    startPolyProcess : function () {
        ...
        if (PolyChrome.prefs.PolyMLPath!="") {
            args.push(PolyChrome.prefs.PolyMLPath.value);
        }
    ...
    destroy : function() {
        //PolyChrome.debug.writeProfilingReport();
        try {
    ...
    setStatus : function(s) {
        this.status = s;
        PolyChrome.browserUI.setStatus(this);
    },
        ...
        for (var i in this.queue) {
            ...
            switch (p.type) {
                case "code":
                    ...
                    this.poly.sendCode("val _=PolyChrome.code_location:=\""+url+"\";");
                    this.poly.sendCode(p.code);
                    break;
                case "sml":
                    this.poly.sendCode('PolyChrome.use "' + p.filename + '";');
                    break;
                case "zip":
...
Socket1.prototype = {
        ...
        do {
            ...
            if (!this.reading) {
                try {
                    ...
                    var r = this.sin.read(PolyChrome.PREFIX_SIZE);
                    if (r.length<PolyChrome.PREFIX_SIZE) {
                ...
                this.bytesNextChunk = this.bytesLeft > PolyChrome.CHUNK_SIZE ?
                                      PolyChrome.CHUNK_SIZE : this.bytesLeft;
                this.reading = true;
            } else {
                try {
                ...
                if (this.bytesLeft == 0) {
                    ...
                    this.reading = false;
                    this.request = "";
                    var bytesToWaitFor = PolyChrome.PREFIX_SIZE;
                } else {
    ...
    onSocketAccepted : function(serverSocket, clientSocket) {
        ...
        this.input.asyncWait(this,0,PolyChrome.PREFIX_SIZE,this.tm.mainThread);
    ...
    send : function(data) {
        try {
            var prefix = (data.length.toString() +
                    Array(PolyChrome.PREFIX_SIZE).join(" "))
                    .substring(0, PolyChrome.PREFIX_SIZE);
            var prefixed_data = prefix + data;
            var pos = 0;
            while (pos<prefixed_data.length) {
                var chunk = prefixed_data.substr(pos, PolyChrome.CHUNK_SIZE);
